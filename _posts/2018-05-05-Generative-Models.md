...
This encoder networks our posterior probability - $$ p_\theta(z \vert x) $$
...
$$\log p_\theta(x) = E_{z~q_\phi(z \vert x} [ \log p_\theta(x) ]$$ 

We're taking the expectation with respect to $$z$$, as $$p_\theta(x)$$ does not depends on $$z$$

We can rewritethe right hand side of this equation using Bayes rule

$$ = E_z [\log \frac{ p_\theta (x \vert z) p_\theta(z) }{ p_\theta(z | x)} ]$$
